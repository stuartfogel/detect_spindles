% me_eegplot() - Scroll (horizontally and/or vertically) through multichannel data.
...
%        % Check  consistency of freqs
  ...
  defctrlupcom = ''; % CTRL press and up -> do nothing by default
   ...
   try, g.winlength; 		catch, g.winlength	= 30; 	end; % Number of seconds of EEG displayed
...
%  set(figh, 'busyaction', 'cancel');
  ...
  if ~isempty(g.events)
      ...
      % g.eventcolors     = { 'r', [0 0.8 0], 'm', 'c', 'k', 'b', [0 0.8 0] }; SF: OLD retina-burning matlab color scheme.
      g.eventcolors     = { [0 0.4470 0.7410], [0.8500 0.3250 0.0980], [0.9290 0.6940 0.1250], [0.4940 0.1840 0.5560], [0.4660 0.6740 0.1880], [0.3010 0.7450 0.9330], [0.6350 0.0780 0.1840] }; % SF: NEW matlab color scheme.
      g.eventstyle      = { '-' '-' '-'  '-'  '-' '-' '-' '--' '--' '--'  '--' '--' '--' '--'};
      g.eventwidths     = [ 2.5 1 ];
...
%             ylims=ylim;
            ...
            if g.ploteventdur & ~isempty(g.eventlatencyend) ...
                ...
                if tmplatend ~= 0, 
                    ...
                    set(h, 'alpha',0.5) % SF: added transparency, but doesn't seem to work!!!
                end;
            end;
         end;
    else % JavierLC
